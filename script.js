const handleCategory = async () => {
    ...
    data.data.slice(0,4).forEach((categories) => {
        const div = document.createElement("div");
        div.innerHTML = `
        <a onclick="handleLoadAll('${categories.category_id
        }')" class="btn">${categories.category}</a> 
        `;
...
const handleLoadAll = async (categoryId) => {
    ...
    data.data.forEach((data) =>{
        ...
        <div class="card w-90 bg-base-100 shadow-xl">
                <figure>
                <div class="w-full h-48 containe">
                <img src="${data.thumbnail
                }" >
                <div class="bottom-right">${data.others.posted_date}</div>
                </div>
                ...
                <div >
                  <img class="w-8 rounded-full"  src="${data.authors[0].profile_picture}"> 
                  </div>
...
function convertTimeToHMS(timeInSeconds) {
